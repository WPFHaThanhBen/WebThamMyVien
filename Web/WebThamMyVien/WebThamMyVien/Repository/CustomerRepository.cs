...
namespace WebThamMyVien.Repository
{
    public class CustomerRepository : ICustomerRepository
    {
        ...
        public CustomerRepository(HttpClient httpClient, IOptions<ConnectAPI> connectionStrings)
        {
        ...
        public async Task<bool> CreateCustomer(CustomerDto _Customer)
        {
            try
            {
                var apiUrl = $"{_apiUrl}/api/Customer";

                // Chuyển dữ liệu của CustomerDto thành JSON
        ...
        public async Task<bool> DeleteCustomer(CustomerDto _Customer)
        {
            var apiUrl = $"{_apiUrl}/api/Customer/{_Customer.Id}"; // Điền đường dẫn API tại đây
        ...
        public async Task<bool> UpdateCustomer(CustomerDto _Customer)
        {
            var apiUrl = $"{_apiUrl}/api/Customer/{_Customer.Id}"; // Điền đường dẫn API tại đây
            var content = new StringContent(JsonConvert.SerializeObject(_Customer), Encoding.UTF8, "application/json");
        ...
        {
            var apiUrl = $"{_apiUrl}/api/Customer/{id}"; // Điền đường dẫn API tại đây
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var values = JsonConvert.DeserializeObject<CustomerDto>(content);
        ...
        public async Task<ICollection<CustomerDto>> GetAllCustomer()
        {
            var apiUrl = $"{_apiUrl}/api/Customer"; // Điền đường dẫn API tại đây
		...
		public async Task<CustomerDto> GetCustomerFinal()
		{
			var apiUrl = $"{_apiUrl}/api/Customer/Final"; // Điền đường dẫn API tại đây
