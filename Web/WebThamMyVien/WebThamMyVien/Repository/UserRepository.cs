...
namespace WebThamMyVien.Repository
{
    public class UserRepository : IUserRepository
    {
        ...
        public UserRepository(HttpClient httpClient, IOptions<ConnectAPI> connectionStrings)
        {
        ...
        public async Task<bool> CreateUser(UserDto _User)
        {
            try
            {
                var apiUrl = $"{_apiUrl}/api/User";

                // Chuyển dữ liệu của UserDto thành JSON
        ...
        {
            var apiUrl = $"{_apiUrl}/api/User/{_User.Id}"; // Điền đường dẫn API tại đây
        ...
        public async Task<bool> UpdateUser(UserDto _User)
        {
            var apiUrl = $"{_apiUrl}/api/User/{_User.Id}"; // Điền đường dẫn API tại đây
            var content = new StringContent(JsonConvert.SerializeObject(_User), Encoding.UTF8, "application/json");
        ...
        {
            var apiUrl = $"{_apiUrl}/api/User/{id}"; // Điền đường dẫn API tại đây
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var values = JsonConvert.DeserializeObject<UserDto>(content);
        ...
        {
            var apiUrl = $"{_apiUrl}/api/User"; // Điền đường dẫn API tại đây
            ...
            {
                var content = await response.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<ICollection<UserDto>>(content);
