...
namespace WebThamMyVien.Repository
{
    public class InvoiceDetailRepository : IInvoiceDetailRepository
    {
        ...
        public InvoiceDetailRepository(HttpClient httpClient, IOptions<ConnectAPI> connectionStrings)
        {
        ...
        public async Task<bool> CreateInvoiceDetail(InvoiceDetailDto _InvoiceDetail)
        {
            try
            {
                var apiUrl = $"{_apiUrl}/api/InvoiceDetail";

                // Chuyển dữ liệu của InvoiceDetailDto thành JSON
                var content = new StringContent(JsonConvert.SerializeObject(_InvoiceDetail), Encoding.UTF8, "application/json");
        ...
        public async Task<bool> DeleteInvoiceDetail(InvoiceDetailDto _InvoiceDetail)
        {
            var apiUrl = $"{_apiUrl}/api/InvoiceDetail/{_InvoiceDetail.Id}"; // Điền đường dẫn API tại đây
        ...
        public async Task<bool> UpdateInvoiceDetail(InvoiceDetailDto _InvoiceDetail)
        {
            var apiUrl = $"{_apiUrl}/api/InvoiceDetail/{_InvoiceDetail.Id}"; // Điền đường dẫn API tại đây
            var content = new StringContent(JsonConvert.SerializeObject(_InvoiceDetail), Encoding.UTF8, "application/json");
        ...
        public async Task<InvoiceDetailDto> GetInvoiceDetail(int id)
        {
            var apiUrl = $"{_apiUrl}/api/InvoiceDetail/{id}"; // Điền đường dẫn API tại đây
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var values = JsonConvert.DeserializeObject<InvoiceDetailDto>(content);
