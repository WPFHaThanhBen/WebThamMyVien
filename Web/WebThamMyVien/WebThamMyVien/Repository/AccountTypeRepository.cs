...
namespace WebThamMyVien.Repository
{
    public class AccountTypeRepository : IAccountTypeRepository
    {
        ...
        public async Task<bool> CreateAccountType(AccountTypeDto _AccountType)
        {
            try
            {
                var apiUrl = $"{_apiUrl}/api/AccountType";

                // Chuyển dữ liệu của AccountTypeDto thành JSON
                var content = new StringContent(JsonConvert.SerializeObject(_AccountType), Encoding.UTF8, "application/json");
        ...
        public async Task<bool> DeleteAccountType(AccountTypeDto _AccountType)
        {
            var apiUrl = $"{_apiUrl}/api/AccountType/{_AccountType.Id}"; // Điền đường dẫn API tại đây
        ...
        public async Task<bool> UpdateAccountType(AccountTypeDto _AccountType)
        {
            var apiUrl = $"{_apiUrl}/api/AccountType/{_AccountType.Id}"; // Điền đường dẫn API tại đây
            var content = new StringContent(JsonConvert.SerializeObject(_AccountType), Encoding.UTF8, "application/json");
        ...
        public async Task<AccountTypeDto> GetAccountType(int id)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var values = JsonConvert.DeserializeObject<AccountTypeDto>(content);
        ...
        public async Task<ICollection<AccountTypeDto>> GetAllAccountType()
        {
            ...
            {
                var content = await response.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<ICollection<AccountTypeDto>>(content);
