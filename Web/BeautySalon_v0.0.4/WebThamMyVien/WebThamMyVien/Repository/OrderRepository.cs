...
namespace WebThamMyVien.Repository
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public OrderRepository(HttpClient httpClient, IOptions<ConnectAPI> connectionStrings)
        {
        ...
        public async Task<bool> CreateOrder(OrderDto _Order)
        {
            try
            {
                var apiUrl = $"{_apiUrl}/api/Order";

                // Chuyển dữ liệu của OrderDto thành JSON
                var content = new StringContent(JsonConvert.SerializeObject(_Order), Encoding.UTF8, "application/json");
        ...
        public async Task<bool> DeleteOrder(OrderDto _Order)
        {
            var apiUrl = $"{_apiUrl}/api/Order/{_Order.Id}"; // Điền đường dẫn API tại đây
        ...
        public async Task<bool> UpdateOrder(OrderDto _Order)
        {
            var apiUrl = $"{_apiUrl}/api/Order/{_Order.Id}"; // Điền đường dẫn API tại đây
            var content = new StringContent(JsonConvert.SerializeObject(_Order), Encoding.UTF8, "application/json");
        ...
        {
            var apiUrl = $"{_apiUrl}/api/Order/{id}"; // Điền đường dẫn API tại đây
        ...
        public async Task<ICollection<OrderDto>> GetAllOrder()
        {
            var apiUrl = $"{_apiUrl}/api/Order"; // Điền đường dẫn API tại đây
